...
class RunManager:
    ...
    def __init__(self, path, net, run_config, init=True, measure_latency=None, no_gpu=False):
        ...
        reprogram_params = [
            ...
            if 'sidemodules' in name
        ]
        print('if has mask real in optimizer==================')

        main_params = [
            param for name, param in net.named_parameters()
            if (param.requires_grad) and ('mask_real' not in name and param.requires_grad) and ('sidemodules' not in name and param.requires_grad) 
        ]
        # used for mask
        # mask_params = [
        #     param for name, param in net.named_parameters()
        ...
        if self.run_config.dataset == 'imagenet':
            ...
            #                         {'params': mask_params, 'lr': self.run_config.init_lr_m, 'weight_decay':0},
            #                         ],
        ...
        for epoch in range(self.start_epoch, self.run_config.n_epochs + warmup_epoch):
            ...
            if (epoch + 1) % self.run_config.validation_frequency == 0:
                ...
                for idx, module in enumerate(self.net.modules()):
                    if 'MyMaskConv2d' in str(type(module)):
                        if hasattr(module, 'mask_real'):
                            # print(idx, module)
                            num_mask = module.mask_real.numel()
                            # print(module.mask_real)
                            # print(module.mask_real.requires_grad)
                            # print(module.mask_real.grad)
                            # num_one = module.bin_mask.data.eq(1).sum().item()
                            num_one = module.total_non_zeros
                            #print(num_mask, num_one)
                            total_mask += num_mask
                            total_mask_one += num_one
                            print('{},{}, -- sparsity:{:2.4f}'.format(idx, module.weight.size(), 1-num_one/num_mask))

                            # print('{},{}, -- sparsity:{:2.4f} --Mean grad:{:2.6f}'.format(idx, module.weight.size(), 1-num_one/num_mask, module.mask_real.grad.abs().mean().data.item()))
                print('total sparsity:{:2.4f}'.format(1-total_mask_one/total_mask))
                mask_log = 'total sparsity:{:2.4f}'.format(1-total_mask_one/total_mask)
                self.write_log(mask_log, prefix='mask', should_print=False)
                '''
                # adding tensorboard
