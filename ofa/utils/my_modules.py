...
__all__ = ['MyModule', 'MyNetwork', 'init_models', 'set_bn_param', 'get_bn_param', 'replace_bn_with_gn',
           'MyConv2d', 'MyMaskConv2d','replace_conv2d_with_my_conv2d', 'replace_conv2d_with_my_maskconv2d']
	...
	for m in model.modules():
		...
		for name, sub_m in m.named_children():
			if isinstance(sub_m, nn.BatchNorm2d):
				...
				gn_m = nn.GroupNorm(num_groups=num_groups, num_channels=sub_m.num_features, eps=0.001, affine=True)
...
def replace_conv2d_with_my_maskconv2d(net, ws_eps=None):
	if ws_eps is None:
		return
	mask_flag = True
	for idx, (n, m) in enumerate(net.named_modules()):
		# to_update_dict = {}
		print(idx, n, m)
		if 'main_branch.blocks.21' in n or 'main_branch.blocks.1.' in n or 'main_branch.blocks.5' in n or 'main_branch.blocks.9' in n or 'main_branch.blocks.13' in n or 'main_branch.blocks.17' in n:
			print(n)
			to_update_dict = {}
			for name, sub_module in m.named_children():
				if isinstance(sub_module, nn.Conv2d) and not sub_module.bias:
			...
			for name, sub_module in to_update_dict.items():
				...
				m._modules[name] = MyMaskConv2d(
					sub_module.in_channels, sub_module.out_channels, sub_module.kernel_size, sub_module.stride,
					sub_module.padding, sub_module.dilation, sub_module.groups, sub_module.bias, mask = mask_flag
				)
				# load weight
				m._modules[name].weight = sub_module.weight

				m._modules[name].weight.requires_grad = sub_module.weight.requires_grad
				if sub_module.bias is not None:
					m._modules[name].bias = sub_module.bias
					m._modules[name].bias.requires_grad = sub_module.bias.requires_grad
		else:
			to_update_dict = {}
			for name, sub_module in m.named_children():
...
class MyMaskConv2d(nn.Conv2d):
	...
	def __init__(self, in_channels, out_channels, kernel_size, stride=1,
	             padding=0, dilation=1, groups=1, bias=True, mask=True, mask_type='kernel_wise', mask_scale=1e-2):
		super(MyMaskConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias)
		self.WS_EPS = None
		self.mask = mask
		self.mask_type = mask_type
		#define mask
		if self.mask_type == 'kernel_wise':
			# print(self.weight.shape)
			# self.mask_real = self.weight.data.new(self.out_channels, self.in_channels, 1, 1)
		...
		self.mask_real = nn.parameter.Parameter(self.mask_real,requires_grad=True)
		self.temperature = 10
		self.Beta = 1200
	...
	def forward(self, x):
		...
		# print(self.mask)
		if self.mask:
			bin_mask = _gumbel_sigmoid((self.mask_real - DEFAULT_THRESHOLD) * self.Beta, gumbel_temp=self.temperature, training=self.training)
			# bin_mask = one_hot
			self.total_non_zeros = bin_mask.sum()
			if self.mask == 'kernel_wise':
				masked_weight = bin_mask.reshape(self.weight.size(0), self.weight.size(1),1,1) * self.weight_standardization(self.weight)    
			else:
				masked_weight = bin_mask * self.weight_standardization(self.weight)
	...
	def __repr__(self):
		return super(MyMaskConv2d, self).__repr__()[:-1] + ', ws_eps=%s)' % self.WS_EPS
