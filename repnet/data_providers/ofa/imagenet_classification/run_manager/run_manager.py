...
class RunManager:
    ...
    def __init__(self, path, net, run_config, init=True, measure_latency=None, no_gpu=False):
        ...
        reprogram_params = [
            ...
            if 'reprogram.p' in name or 'encodermodule' in name 
            # if 'reprogram.p' in name
        ]
        # for name, param in net.named_parameters():
        #     if 'reprogram.p' in name or 'encodermodule' in name:
        #         print(name, param.shape)
        #         print('=========')
            # if ('reprogram.p' not in name and param.requires_grad) and ('encodermodule' not in name and param.requires_grad):
            #     print(name, param.shape)
        main_params = [
            param for name, param in net.named_parameters()
            if ('reprogram.p' not in name and param.requires_grad) and ('encodermodule' not in name and param.requires_grad) 
            # if 'reprogram.p' not in name and param.requires_grad
        ]
        # net_params = 
        # optimizer = torch.optim.Adam(net_params, init_lr)
        self.optimizer = torch.optim.Adam([
                                   {'params': reprogram_params, 'lr': self.run_config.init_lr_p, 'weight_decay':0},
                                   {'params': main_params, 'weight_decay': 0}
                                   ],
        ...
        #                             {'params': main_params, 'weight_decay': 0}
        #                             ],
        #                             lr=self.run_config.init_lr,momentum=0.9,nesterov=True)       
    ...
    def train_one_epoch(self, args, epoch, warmup_epochs=0, warmup_lr=0):
        ...
        # print p
        for n, v in self.net.named_parameters():
            if 'reprogram.p' in n:
                print(n, v.min().item(), v.max().item(),v.abs().mean().item())
                print(v)
                print(v.grad)

        return losses.avg, self.get_metric_vals(metric_dict)

    def train(self, args, writer, warmup_epoch=0, warmup_lr=0):
        for epoch in range(self.start_epoch, self.run_config.n_epochs + warmup_epoch):
            ...
            if (epoch + 1) % self.run_config.validation_frequency == 0:
                ...
                # p_log = 'reprogram grad flow {}\treprogram flow {}'. \
                #     format(np.array(self.net.module.reprogram.p.grad.data.norm().cpu()), np.array(self.net.module.reprogram.p.data.norm().cpu()))
                self.write_log(val_log, prefix='valid', should_print=False)
                # self.write_log(p_log, prefix='reprogram', should_print=False)

                # adding tensorboard
                writer.add_scalar('train loss', train_loss, epoch + 1)
                writer.add_scalar('train acc', train_top1, epoch + 1)
                writer.add_scalar('test loss', val_loss[0], epoch + 1)
                writer.add_scalar('test acc', val_acc[0], epoch + 1)
                # writer.add_histogram("reprogram.p", self.net.module.reprogram.p.data, epoch + 1)
                # writer.add_histogram("reprogram.p.grad", self.net.module.reprogram.p.grad, epoch + 1)
                # writer.add_scalar("reprogram grad flow", self.net.module.reprogram.p.grad.norm(), epoch + 1)
                # writer.add_scalar("reprogram flow", self.net.module.reprogram.p.norm(), epoch + 1)
                # print('reprogram grad flow: {}, reprogram flow: {}'. \
                #     format(self.net.module.reprogram.p.grad.norm(), self.net.module.reprogram.p.norm()))
            else:
                is_best = False
